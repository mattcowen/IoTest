...
workflow Ramp-Test
{
	...
	foreach -parallel ($testname in $tests){
	    ...
	    [String]$resourceGroupNamePrefix = 'Test'
	    ...
	    [String]$vmNamePrefix = $testname # DO NOT USE CHARS SUCH AS HYPHENS
        ...
        for ($x = 0; $x -le $totalVmCount; $x++)
        {
            #Write-Host "Starting $x"
            $guid = [System.Guid]::NewGuid().ToString().Replace('-', '').substring(0, 8)
            $resourceGroup = $resourceGroupNamePrefix + $guid
            ...
            <#$job = Start-Job -ScriptBlock { 
                ...
                $vmName = "$vmNamePrefix$guid"
                $testName = "$vmNamePrefix$guid"
